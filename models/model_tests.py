...
class TestModelSetup:
    ...
    def test_repr(self):
        ...
        assert 'Bayesian' in repr(CommonalityModel(k=4, lam=0.0))
        assert 'Egocentric' in repr(CommonalityModel(lam=1.0))
        assert 'λ=0.50' in repr(CommonalityModel(k=4, lam=0.5))
...
class TestModelBehavior:
    ...
    def test_lambda_affects_predictions(self):
        """λ=0 (Bayesian) vs λ=1 (Egocentric) should give different predictions."""
        r_self = np.array([1.0] * 17 + [5.0] * 18)  # Polarized responses
        preds_bayes = CommonalityModel(k=5, lam=0.0).predict(0, 3.0, r_self)
        preds_ego = CommonalityModel(k=5, lam=1.0).predict(0, 3.0, r_self)
        assert not np.allclose(preds_bayes, preds_ego)

    def test_lambda_clipped(self):
        """λ should be clipped to [0, 1]."""
        assert CommonalityModel(lam=1.5).lam == 1.0
        assert CommonalityModel(lam=-0.5).lam == 0.0

    def test_lambda_mixture_interpolates(self):
        """λ=0.5 should give predictions between λ=0 and λ=1."""
        r_self = np.random.uniform(1, 5, 35)
        preds_0 = CommonalityModel(k=5, lam=0.0, epsilon=0.0).predict(0, 3.0, r_self)
        preds_1 = CommonalityModel(k=5, lam=1.0, epsilon=0.0).predict(0, 3.0, r_self)
        preds_half = CommonalityModel(k=5, lam=0.5, epsilon=0.0).predict(0, 3.0, r_self)
        # Mixture should be close to average
        expected = 0.5 * preds_0 + 0.5 * preds_1
        assert np.allclose(preds_half, expected, atol=0.01)

    def test_egocentric_uses_self_similarity(self):
        """Egocentric model should predict higher match for similar self-responses."""
        # Self responds identically on questions 0 and 1, differently on question 2
        r_self = np.array([3.0, 3.0, 1.0] + [3.0] * 32)
        model = CommonalityModel(lam=1.0, epsilon=0.0)  # Pure egocentric
        preds = model.predict(0, 3.5, r_self)  # Observe match on q0
        # q1 (same self-response as q0) should have higher prediction than q2
        assert preds[1] > preds[2]

    def test_k5_creates_gradient(self):
        """With k>0, predictions should vary across questions."""
        model = CommonalityModel(k=5, lam=0.0, epsilon=0.0)
        r_self = np.random.uniform(1, 5, 35)
