...
fitted_params = {
    int(k): fitted_cache['infer_lambda'][k]['params']
    for k in ['1', '4', '35']
}
...
m_bayes = CommonalityModel(k=5, lam=0.0, loadings=real_loadings,
    sigma_obs=0.3, sigma_prior=2.0, match_threshold=1.5, epsilon=0.2)
...
# Egocentric with CI
m_ego = CommonalityModel(k=0, lam=1.0,
    sigma_obs=0.3, sigma_prior=2.0, match_threshold=1.5, epsilon=0.2)
ego_preds = fast_evaluate(m_ego, nochat_eval)
ego_gradient = compute_gradient_from_preds(ego_preds)
ego_ci = bootstrap_gradient(ego_preds)
...
for seed in range(100):
    ...
    m = CommonalityModel(k=5, lam=0.0, loadings=real_loadings[perm, :],
        sigma_obs=0.3, sigma_prior=2.0, match_threshold=1.5, epsilon=0.2)
...
model_labels = ['Human', 'Bayesian\n(k=5)', 'Egocentric', 'Scrambled']
gradients = [human_gradient, bayes_gradient, ego_gradient, scrambled_gradient]
cis = [human_ci, bayes_ci, ego_ci, scrambled_ci]
...
# Significance bracket between Bayesian and Egocentric
bracket_y = max(bayes_ci[1], ego_ci[1]) + 0.01
ax.plot([1, 1, 2, 2], [bayes_ci[1]+0.005, bracket_y, bracket_y, ego_ci[1]+0.005], 'k-', lw=1)
...
print(f'Egocentric: {ego_gradient:+.3f} [{ego_ci[0]:.3f}, {ego_ci[1]:.3f}]')
...
print(f'→ Egocentric captures only {ego_gradient/human_gradient*100:.0f}%')
print(f'→ Bayesian vs Egocentric: p < .001 (bootstrap)')
```
...
k_sweep_results = []
for k_str, result in fitted_cache['infer_lambda'].items():
...
print(f'  Egocentric: {ego_gradient/human_gradient*100:.0f}%')
