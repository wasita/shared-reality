...
def compute_self_structure_gradient(responses):
    """Compute within - cross domain similarity from individual responses."""
    within_sims, cross_sims = [], []
    for d, (start, end) in DOMAIN_RANGES.items():
        for i in range(start, end):
            for j in range(i+1, end):
                within_sims.append(1 - abs(responses[i] - responses[j]) / 4.0)
        for d2, (s2, e2) in DOMAIN_RANGES.items():
            if d != d2:
                for i in range(start, end):
                    for j in range(s2, e2):
                        cross_sims.append(1 - abs(responses[i] - responses[j]) / 4.0)
    return np.mean(within_sims) - np.mean(cross_sims)

# Compute self-structure for each participant
structure_gradients = {}
for pid in response_matrix.index:
    responses = response_matrix.loc[pid].values
    structure_gradients[pid] = compute_self_structure_gradient(responses)
...
def compute_group_gradient(df, col):
    rates = {}
    for qt in ['same_domain', 'different_domain']:
        for mt in ['high', 'low']:
...
normal_gradient = compute_group_gradient(normal_data, 'participant_binary_prediction')
anomaly_gradient = compute_group_gradient(anomaly_data, 'participant_binary_prediction')

# Bootstrap CIs
def bootstrap_gradient(df, col, n_boot=1000, seed=42):
    np.random.seed(seed)
    grads = []
    pids = df['pid'].unique()
    for _ in range(n_boot):
        ...
        g = compute_group_gradient(boot_df, col)
        if not np.isnan(g):
            grads.append(g)
...
ax = axes[0]
ax.hist(all_grads, bins=25, color=COLOR_POP, alpha=0.8, edgecolor='white', linewidth=0.5)
ax.axvline(q25, color=COLOR_SELF, linestyle='--', linewidth=2,
           label=f'Bottom quartile\n(N={n_anomaly})')
ax.axvline(0, color='gray', linestyle=':', linewidth=1, alpha=0.7)
ax.axvspan(min(all_grads) - 0.01, q25, alpha=0.15, color=COLOR_SELF)
...
ax.bar([0], [normal_gradient], width=width, color=COLOR_POP,
       yerr=normal_err, capsize=4, error_kw={'linewidth': 1.5})
ax.bar([1], [anomaly_gradient], width=width, color=COLOR_SELF,
